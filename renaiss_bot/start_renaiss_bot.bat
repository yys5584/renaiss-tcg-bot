...
set "RENAISS_LOG_PATH=%RENAISS_LOG_DIR%\renaiss_bot_service.log"
...
"%RENAISS_PYTHON_EXE%" -B -E -s -u -m renaiss_bot.tools.stream_runner --source-root "%RENAISS_ROOT%" --log-path "%RENAISS_LOG_PATH%" --max-bytes "%RENAISS_LOG_MAX_BYTES%" --backups "%RENAISS_LOG_BACKUPS%" --module renaiss_bot.main
set "RC=%ERRORLEVEL%"
popd
exit /b %RC%
