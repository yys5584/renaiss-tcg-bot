...
if not defined RENAISS_ENV_FILE (
  if not defined ProgramData (
    >&2 echo Renaiss launcher refused: set RENAISS_ENV_FILE to a pre-created absolute web-only env file.
    exit /b 2
  )
  set "RENAISS_ENV_FILE=%ProgramData%\Renaiss\secrets\web.env"
)
if not "%RENAISS_ENV_FILE:~1,2%"==":\" (
  >&2 echo Renaiss launcher refused: RENAISS_ENV_FILE must be an absolute local file.
  exit /b 2
)
if not exist "%RENAISS_ENV_FILE%" (
  >&2 echo Renaiss launcher refused: pre-create the web-only RENAISS_ENV_FILE outside the source tree.
  exit /b 2
)
"%RENAISS_PYTHON_EXE%" -B -E -s -c "from pathlib import Path; import sys; root = Path(sys.argv[1]).resolve(); env_file = Path(sys.argv[2]).resolve(); raise SystemExit(env_file == root or root in env_file.parents or not env_file.is_file())" "%RENAISS_ROOT%" "%RENAISS_ENV_FILE%" >nul 2>&1
if errorlevel 1 (
  >&2 echo Renaiss launcher refused: RENAISS_ENV_FILE must be a regular file outside the source tree.
  exit /b 2
)
...
set "RENAISS_LOG_PATH=%RENAISS_LOG_DIR%\renaiss_web_service.log"
...
"%RENAISS_PYTHON_EXE%" -B -E -s -u -m renaiss_bot.tools.stream_runner --source-root "%RENAISS_ROOT%" --log-path "%RENAISS_LOG_PATH%" --max-bytes "%RENAISS_LOG_MAX_BYTES%" --backups "%RENAISS_LOG_BACKUPS%" --module renaiss_bot.web.app
set "RC=%ERRORLEVEL%"
popd
exit /b %RC%
